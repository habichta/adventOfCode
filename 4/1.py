...
lazy_guard, guards_dict, guard_state, time_start,curr_guard =-1, co.defaultdict(lambda: co.defaultdict(int)), 0, 0,-1
with open('input.txt') as f: #O(n log(n))
...
print(lazy_guard* max(guards_dict[lazy_guard].items(), key=operator.itemgetter(1))[0])
